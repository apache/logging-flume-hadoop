...
public class AvroKuduOperationsProducer implements KuduOperationsProducer {
     ...
     */
    private static final LoadingCache<String, Schema> schemasFromURL = CacheBuilder.newBuilder()
     ...
     */
    private static final LoadingCache<String, Schema> schemasFromLiteral = CacheBuilder.newBuilder()
            .build(new CacheLoader<String, Schema>() {
                @Override
     ...
     */
    private static final LoadingCache<Schema, DatumReader<GenericRecord>> readers = CacheBuilder.newBuilder()
                    ...
                    switch (col.getType()) {
                        ...
                        default:
                            throw new FlumeException(String.format(
                                    "Unrecognized type %s for column %s",
                                    col.getType().toString(), name));
                    }
                } catch (ClassCastException e) {
                    throw new FlumeException(
                            String.format(
                                    "Failed to coerce value for column '%s' to type %s", col.getName(), col.getType()),
                            e);
                }
            }
        }
    ...
    private Schema getSchema(Event event) throws FlumeException {
        ...
        try {
            if (schemaUrl != null) {
                ...
                throw new FlumeException(String.format(
                        "No schema for event. " + "Specify configuration property '%s' or event header '%s'",
                        SCHEMA_PROP, SCHEMA_URL_HEADER));
            }
