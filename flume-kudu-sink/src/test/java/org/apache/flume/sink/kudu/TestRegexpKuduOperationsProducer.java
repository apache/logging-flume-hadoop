...
public class TestRegexpKuduOperationsProducer {
    private static final String TEST_REGEXP = "(?<key>\\d+),(?<byteFld>\\d+),(?<shortFld>\\d+),(?<intFld>\\d+),"
    ...
    private KuduTable createNewTable(String tableName) throws Exception {
        ...
        CreateTableOptions createOptions = new CreateTableOptions()
                .addHashPartitions(ImmutableList.of("key"), 3)
                .setNumReplicas(1);
    ...
    private void testEvents(int eventCount, int perEventRowCount, String operation) throws Exception {
        ...
        context.put(
                KuduSinkConfigurationConstants.PRODUCER_PREFIX + RegexpKuduOperationsProducer.PATTERN_PROP,
                TEST_REGEXP);
        context.put(
                KuduSinkConfigurationConstants.PRODUCER_PREFIX + RegexpKuduOperationsProducer.OPERATION_PROP,
                operation);
        context.put(KuduSinkConfigurationConstants.PRODUCER, RegexpKuduOperationsProducer.class.getName());
    ...
    private List<Event> generateEvents(int eventCount, int perEventRowCount, String operation) {
        ...
        for (int i = 0; i < eventCount; i++) {
            ...
            for (int j = 0; j < perEventRowCount; j++) {
                String baseRow =
                        "|1%1$d%2$d1,%1$d,%1$d,%1$d,%1$d,binary," + "string,false,%1$d.%1$d,%1$d.%1$d,%1$d.%1$d|";
            ...
            if (operation.equals("upsert")) {
                ...
                for (int j = 0; j < perEventRowCount; j++) {
                    String row = String.format(
                            "|1%2$d%3$d1,%1$d,%1$d,%1$d,%1$d,binary," + "string,false,%1$d.%1$d,%1$d.%1$d,%1$d.%1$d|",
                            1, 0, j);
                    upserts.append(row);
                }
