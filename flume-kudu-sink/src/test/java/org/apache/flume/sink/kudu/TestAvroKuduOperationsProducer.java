...
public class TestAvroKuduOperationsProducer {
    ...
    private void testEvents(int eventCount, SchemaLocation schemaLocation) throws Exception {
        ...
        Context context = schemaLocation != SchemaLocation.GLOBAL
                ? new Context()
                : new Context(ImmutableMap.of(
                        KuduSinkConfigurationConstants.PRODUCER_PREFIX + AvroKuduOperationsProducer.SCHEMA_PROP,
                        schemaUriString));
        context.put(KuduSinkConfigurationConstants.PRODUCER, AvroKuduOperationsProducer.class.getName());
    ...
    private List<Event> generateEvents(int eventCount, SchemaLocation schemaLocation) throws Exception {
        ...
        for (int i = 0; i < eventCount; i++) {
            ...
            if (schemaLocation == SchemaLocation.URL) {
                e.setHeaders(ImmutableMap.of(AvroKuduOperationsProducer.SCHEMA_URL_HEADER, schemaUriString));
            } else if (schemaLocation == SchemaLocation.LITERAL) {
                e.setHeaders(ImmutableMap.of(AvroKuduOperationsProducer.SCHEMA_LITERAL_HEADER, schemaLiteral));
            }
            events.add(e);
        }
        return events;
    ...
    private List<String> makeAnswers(int eventCount) {
        ...
        for (int i = 0; i < eventCount; i++) {
            answers.add(String.format(
                    ...
                    10 * i, 2 * i, 2.71828 * i, i % 2 == 0 ? "NULL" : "taco", i));
        }
