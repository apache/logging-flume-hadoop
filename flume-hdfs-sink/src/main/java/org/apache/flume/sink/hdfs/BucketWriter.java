...
class BucketWriter {
    ...
    private Method getRefIsClosed() {
        try {
            ...
            LOG.info("isFileClosed() is not available in the version of the " + "distributed filesystem being used. "
    ...
    private class CloseHandler implements Callable<Void> {
        ...
        public void close(boolean immediate) {
            ...
            try {
                ...
                LOG.warn(
                        "Closing file: " + path + " failed. Will " + "retry again in " + retryInterval + " seconds.",
                        e);
                ...
                if (!shouldRetry) {
                    LOG.warn("Unsuccessfully attempted to close " + path + " " + maxRetries
                            + " times. Initializing lease recovery.");
                    sinkCounter.incrementConnectionFailedCount();
                    recoverLease();
                }
            }
    ...
    private class ScheduledRenameCallable implements Callable<Void> {
        ...
        public Void call() throws Exception {
            if (renameTries >= maxRetries) {
                LOG.warn("Unsuccessfully attempted to rename " + path + " " + maxRetries
            ...
            try {
                ...
                LOG.warn(
                        "Renaming file: " + path + " failed. Will " + "retry again in " + retryInterval + " seconds.",
                        e);
                timedRollerPool.schedule(this, retryInterval, TimeUnit.SECONDS);
                return null;
            }
    ...
    public synchronized void append(final Event event) throws IOException, InterruptedException {
            ...
            if (isUnderReplicated) {
                if (maxConsecUnderReplRotations > 0 && consecutiveUnderReplRotateCount >= maxConsecUnderReplRotations) {
                    ...
                    if (consecutiveUnderReplRotateCount == maxConsecUnderReplRotations) {
                        LOG.error(
                                "Hit max consecutive under-replication rotations ({}); "
        ...
        try {
            ...
            LOG.warn(
                    "Caught IOException writing to HDFSWriter ({}). Closing file (" + bucketPath
